...
def main():
    ...
    result_files = [rf for rf in result_files if os.path.isfile(rf)]
    result_list = []
    for rf in result_files:
        try:
            result_list.append(get_trajectory(rf))
    ...
    # Handle models that don't support temperature=0.0 (like some reasoning models)
    completion_kwargs = {
    ...
    if float(args.temperature) != 0.0:
        completion_kwargs["temperature"] = float(args.temperature)
        
    response = client.chat.completions.create(**completion_kwargs).choices[0].message.content
